...
public class GrafoDirigido extends Grafo {
    ...
    private ListaEnlazada<Adyacencia> listaAdyacencias[];

    public GrafoDirigido(Integer num_vertices) {
        this.num_vertice = num_vertices;
        this.num_aristas = 0;
        this.listaAdyacencias = new ListaEnlazada[num_vertices + 1];
        for (int i = 0; i <= this.num_vertice; i++) {
            listaAdyacencias[i] = new ListaEnlazada<>();
        }
    }
    ...
    public Boolean existe_arista(Integer v1, Integer v2) throws Exception {
        ...
        if (v1.intValue() <= num_vertice && v2.intValue() <= num_vertice) {
            ListaEnlazada<Adyacencia> listaA = listaAdyacencias[v1];
    ...
    public ListaEnlazada<Adyacencia>[] getListaAdyacencias() {
    ...
    public void setListaAdyacencias(ListaEnlazada<Adyacencia>[] listaAdyacencias) {
        this.listaAdyacencias = listaAdyacencias;
    }

    @Override
    public ListaEnlazada<Adyacencia> adyacentes(Integer v1) {
